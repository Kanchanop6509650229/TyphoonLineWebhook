...
# ฟังก์ชันสำหรับการจัดการประวัติการสนทนาแบบไฮบริด
def get_session_token_count(user_id):
    ...
    try:
        ...
        # รวมข้อความทั้งหมดเพื่อคำนวณโทเค็น
        total_text = ""
        for msg in messages:
            if msg.get('role') in ['user', 'assistant']:
                total_text += msg.get('content', '')
        
        # คำนวณและคืนค่าจำนวนโทเค็น
...
def hybrid_context_management(user_id):
    ...
    try:
        ...
        current_tokens = token_counter.count_message_tokens(current_history)
        ...
        # เก็บข้อความ X ล่าสุดเสมอ
        keep_recent = 20  # เก็บ 20 รายการล่าสุด
        ...
        if older_messages:
            ...
            logging.info(f"สำเร็จ: จัดการประวัติการสนทนาสำหรับผู้ใช้ {user_id}, ลดจาก {current_tokens} เหลือ {token_counter.count_message_tokens(new_history)} tokens")
...
def process_conversation_data(user_id, user_message, bot_response, messages):
    ...
    db.save_conversation(
        ...
        important=is_important  # เพิ่มการระบุความสำคัญ
    )
    ...
    token_threshold_warning = TOKEN_THRESHOLD * 0.85  # แจ้งเตือนที่ 85% ของขีดจำกัด
    ...
    if session_token_count > token_threshold_warning and not redis_client.exists(f"token_warning:{user_id}"):
        ...
        # ตั้งค่าเวลาหมดอายุของการแจ้งเตือน (1 ชั่วโมง)
        redis_client.setex(f"token_warning:{user_id}", 3600, "1")
...
def handle_command_with_processing(user_id, command):
    ...
    if command == '/reset':
        ...
        # คำนวณโทเค็นในเซสชัน
        session_tokens = get_session_token_count(user_id)
        status_data['session_tokens'] = session_tokens
        ...
        response_text = (
            ...
            f"▫️ โทเค็นในเซสชันปัจจุบัน: {status_data['session_tokens']:,}\n"
            f"▫️ โทเค็นทั้งหมดที่ใช้: {status_data['total_tokens']:,}\n\n"
